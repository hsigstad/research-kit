...
Replaces §3–6 with a residual sweep for patterns no current finding addresses.
Read [`reference/open-ended-mode.md`](reference/open-ended-mode.md) when this
mode is invoked; otherwise skip.

### 8. Persist results

Three durable outputs: (a) **tagged corpus** — annotate the source rows/files
in place (append `audit_flag` to `stories.csv`; sidecar `.audit.json` for
free-form and reports; never delete prior tags); (b) **audit report** —
git-tracked markdown at `docs/audits/findings/<YYYY-MM-DD>-<mode>.md`, headed by
the source-bias disclaimer; (c) **cache + JSON** — `state.yaml` (findings sha +
per-source signatures for incremental re-audits) and a machine-readable
`<date>-<mode>.json`.

The exact report template, `state.yaml` / JSON schemas, and the verbatim
source-bias disclaimer block are in
[`reference/output-schemas.md`](reference/output-schemas.md) — read it at this
step. Re-audit scoping: findings unchanged + sources unchanged → no work;
findings changed → re-audit changed findings vs full corpus; source signature
changed → new entries vs all findings; both → union.
