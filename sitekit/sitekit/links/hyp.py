...
def link_h_refs(html: str, ctx: BuildContext, current_stem: str = "") -> str:
    """Auto-link bare H:slug tokens to their hypothesis page under hypotheses/."""
    if not ctx.h_slugs:
        return html
    ...
    def _replacer(m: re.Match) -> str:
        ...
        if slug not in ctx.h_slugs or slug == current_stem:
            return token
        return f'<a href="../hypotheses/{slug}.html">{token}</a>'
